...
public class Day05 {
	...
	public static void main(String[] args) throws Exception {
		List<String> lines =
				Files.lines(Paths.get("input.txt")).collect(Collectors.toList());
		long res = part2(lines);
		System.out.println(res);
	}

	static long part1(List<String> codes) {
		return getMax(codes.stream().map(Day05::computeSeatNumber)
				.collect(Collectors.toUnmodifiableSet()));
	}

	// The description of the problem is pretty vague. What we have to do is to find the missing
	// maximum value from
	// the available seat IDs. We know that in total can be 1024 seats on the plane, but some
	// seats from the front or back
	// are missing. We know what is ID of the biggest seat number (from part1). We could also
	// try to find the minimum value,
	// but in the end does not really matter, since our missing seat is somewhere in the middle
	// ("Your seat wasn't at
	// the very front or back, though"). So we iterate from 0 and try to find the biggest value
	// missing from the set.
	static long part2(List<String> codes) {
		Set<Long> seats = codes.stream().map(Day05::computeSeatNumber)
				.collect(Collectors.toUnmodifiableSet());
